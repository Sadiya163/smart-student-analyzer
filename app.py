...
def register():
    if request.method == "POST":
        ...
        if not name or not email or len(password) < 6:
            flash(
                "Enter all fields. Password must contain at least 6 characters.",
                "error"
            )
...
def dashboard():
    try:
        ...
        cursor.execute(
            """
            SELECT id, student_name, average, grade, result,
                   CONVERT_TZ(
                       created_at,
                       '+00:00',
                       '+05:30'
...
def analyze():
    if request.method == "POST":
        student_name = (
            ...
            or session.get("user_name", "Student")
        )
        ...
        if not marks:
            errors.append(
                "Enter at least one valid subject."
            )
...
def report(report_id):
    try:
        ...
        marks = {
            row["subject_name"]: float(row["score"])
            for row in rows
        }
